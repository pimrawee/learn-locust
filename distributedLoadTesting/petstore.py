...
class PetStore(SequentialTaskSet):
    def __init__(self, parent):
        ...
        self.jsession = ""          # Stores jsession ID extracted from response
        self.random_product = ""    # Stores a random product category after login

    @task
    def home_page(self):
        with self.client.get("", catch_response=True, name="T00_HomePage") as response:
            if "Welcome to JPetStore 6" in response.text and response.elapsed.total_seconds() < 2.0:    # Check welcome message and response time
                response.success()
            else:
    ...
    def enter_store(self):
        ...
        with self.client.get("/actions/Catalog.action", catch_response=True, name="T10_EnterStore") as response:
            for product in products:    # Check that all product categories appear in the page
                if product in response.text:
                    ...
                    break
            try:
                jsession = re.search(r"jsessionid=(.+?)\?", response.text)  # Extract jsession ID from response for later use
    ...
    def login(self):
        self.client.cookies.clear()
        # Submit login form using predefined credentials
        ...
        with self.client.post(url, name="T30_SignIn", data=data, catch_response=True) as response:
            if "Welcome ABC!" in response.text:
                response.success()
                try:
                    random_product = re.findall(r"Catalog.action\?viewCategory=&categoryId=(.+?)\"", response.text)  # Extract all product categories and pick one at random
...
# Termical all (master and worker)
# Shell          | Command                        |
# -------------- | ------------------------------ |
...
# Terminal master
# locust --config path/master.conf

# Terminal worker
# locust --config path/worker.conf
